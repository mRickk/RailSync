...
export const createLocksTimeout = async function(req, res) {
    try {
        const { seat, userId } = req.body;
        const solutionId = req.params.solutionId;

        if (!seat || !userId) {
            return res.status(400).json({ message: "seat and userId are required" });
        }
...
export const renewLocksTimeout = async function(req, res) {
    try {
        ...
        const success = await redis.pexpire(lockKey, 30000);
        if (success === 1) {
